...
| **[Brave Search](https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search)** | `01-12` | **Claude Desktop** | `{"mcpServers":{`<br>`"brave-search":{`<br>`"command":"npx",`<br>`"args":["-y",`<br>`"@modelcontext`<br>`protocol/server-`<br>`brave-search"],`<br>`"env":{"BRAVE_`<br>`API_KEY":`<br>`"your_api_key"}}}}` | 🔑 [Brave API](https://brave.com/search/api/) |
